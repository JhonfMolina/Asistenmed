...
import { ExamenesService } from '../../parametros/service/examenes.service';
import { LaboratoriosService } from '../../parametros/service/laboratorios.service';
...
@Component({
  selector: 'app-orden-medica-laboratorio',
  templateUrl: './orden-medica-laboratorio.component.html',
  styleUrls: ['./orden-medica-laboratorio.component.scss']
})
export class OrdenMedicaLaboratorioComponent implements OnInit, OnDestroy {
  ...
  public listadoExamenes:               Array<any> = [];
  public listadoLaboratorios:           Array<any> = [];
  ...
  constructor(
    ...
    this.formularioDetalle = this._formBuilder.group({
      asistencial_examen_id:   ['', Validators.required],
  ...
  inicializador() {
    setTimeout(() => {
      ...
      this.getListadoExamenes();
      this.getListadoLaboratorios();
    }, 0);
  }
  ...
  getListadoLaboratorios() {
    this.subscription.push(this._laboratoriosService.getListadoLaboratorios(this.CENTRO_MEDICO.id).subscribe((resp) => {
      this.listadoLaboratorios = resp.data;
    }));
  }
  ...
  getListadoExamenes() {
    this.subscription.push(this._examenesService.getListadoExamenes(this.CENTRO_MEDICO.id).subscribe((resp) => {
      this.listadoExamenes = resp.data;
    }));
  }
  ...
  pushItemDetalle(){
    ...
    const examen = this.listadoExamenes.find(
      (item) => item.id == this.formDetalleControl()['asistencial_examen_id'].value
    )
    if (this.formularioDetalle.valid) {
      this.detalle.push(
        new DetalleOrdenMedica(
          ...
          '',
          '',
          this.formControl()['asistencial_laboratorio_id'].value,
          ''
        )
      )
